...
export function PdpEditor({
  ...
  type EditorHistoryState = {
    ...
    sectionOptions: typeof sectionOptions;
    overlaysBySection: typeof overlaysBySection;
    defaultCopyLanguage: typeof defaultCopyLanguage;
  };
  ...
  const historyRef = useRef<{
    ...
    committed: EditorHistoryState | null;
    pendingLive: EditorHistoryState | null;
  ...
  const flushPendingHistoryCommit = useCallback(() => {
    ...
    if (!hist.commitTimer) return;
    clearTimeout(hist.commitTimer);
    hist.commitTimer = null;
    if (hist.pendingLive && hist.committed) {
      hist.past.push(hist.committed);
      if (hist.past.length > HISTORY_LIMIT) hist.past.shift();
      hist.future = [];
      hist.committed = hist.pendingLive;
      hist.pendingLive = null;
      setHistoryVersion((v) => v + 1);
    }
  }, []);

  useEffect(() => {
    const live: EditorHistoryState = {
      sections,
      sectionOptions,
      overlaysBySection,
      defaultCopyLanguage
    ...
    if (hist.committed === null) {
      hist.committed = live;
      return;
    }

    const unchanged =
      hist.committed.sections === sections &&
      hist.committed.sectionOptions === sectionOptions &&
      hist.committed.overlaysBySection === overlaysBySection &&
      hist.committed.defaultCopyLanguage === defaultCopyLanguage;
    if (unchanged) return;

    // 새 편집이 시작되면 redo 체인을 즉시 무효화해 버튼 상태를 갱신한다.
    if (hist.future.length > 0) {
      hist.future = [];
      setHistoryVersion((v) => v + 1);
    }

    hist.pendingLive = live;
    if (hist.commitTimer) clearTimeout(hist.commitTimer);
    hist.commitTimer = setTimeout(() => {
      if (!hist.pendingLive || !hist.committed) {
        hist.commitTimer = null;
        return;
      }
  ...
  }, [sections, sectionOptions, overlaysBySection, defaultCopyLanguage]);

  const applyHistorySnapshot = useCallback((snapshot: EditorHistoryState) => {
    historyRef.current.isApplying = true;
    setSections(snapshot.sections);
    setSectionOptions(snapshot.sectionOptions);
    setOverlaysBySection(snapshot.overlaysBySection);
    setDefaultCopyLanguage(snapshot.defaultCopyLanguage);
  ...
  const handleUndo = useCallback(() => {
    ...
    const previous = hist.past.pop() as EditorHistoryState;
    if (hist.committed) hist.future.push(hist.committed);
    hist.committed = previous;
    applyHistorySnapshot(previous);
    setHistoryVersion((v) => v + 1);
  }, [applyHistorySnapshot, flushPendingHistoryCommit]);

  const handleRedo = useCallback(() => {
    flushPendingHistoryCommit();
    const hist = historyRef.current;
    if (hist.future.length === 0) return;
    const next = hist.future.pop() as EditorHistoryState;
    if (hist.committed) hist.past.push(hist.committed);
    hist.committed = next;
    applyHistorySnapshot(next);
    setHistoryVersion((v) => v + 1);
  }, [applyHistorySnapshot, flushPendingHistoryCommit]);

  const canUndo = historyRef.current.past.length > 0 || historyRef.current.pendingLive !== null;
  const canRedo = historyRef.current.future.length > 0;
  void historyVersion; // re-render trigger when history state changes
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      // 입력 필드에서는 브라우저 기본 undo를 방해하지 않도록 차단
  ...
  // 사용자 요청: 플로팅 워크벤치를 항상 캔버스 우측에 앵커링하고
  // 세로 높이를 캔버스에 맞추어 최대화 (상하 16px 여백)
  useEffect(() => {
    ...
    setWorkbenchState((current) => {
      ...
      const nextHeight = Math.max(420, stageHeight - 32);
      const nextWidth = current.width;
      const maxX = Math.max(16, stageWidth - nextWidth - 16);
      return {
        ...current,
        x: maxX,
        y: 16,
            ...
            <article className={styles.canvasPanel}>
              <div className={styles.canvasHeader}>
                ...
                <div className={styles.canvasActions}>
                  ...
                  <span className={styles.canvasActionsDivider} aria-hidden="true" />
                  <button
                    className={styles.navButton}
...
function getWorkbenchPosition(stageEl: HTMLDivElement | null) {
  ...
  const stageWidth = stageEl?.clientWidth ?? 1240;
  const stageHeight = stageEl?.clientHeight ?? 720;
  const height = Math.max(420, stageHeight - 32);
